        ...
        try {
            ...
            for (Map.Entry<String, Object> entry : iocMap.entrySet()) {
                ...
                for (Field field : fields) {
                    if(field.isAnnotationPresent(YphAutoWired.class)){
                        //获取注解中的value值|该值就是bean的name
                        YphAutoWired autoWiredAno =  field.getAnnotation(YphAutoWired.class);
    ...
    public void executeInstance(){
        ...
        try{
            ...
            for (String className : classNameList) {
                ...
                if(clazz.isAnnotationPresent(YphController.class)){
                    //控制层 bean

                    String beanName = clazz.getSimpleName().substring(0,1).toLowerCase()+ clazz.getSimpleName().substring(1);
                    iocMap.put(beanName,clazz.newInstance());

                }else if(clazz.isAnnotationPresent(YphService.class)){
                    //Service层  bean
                    YphService serviceAn = clazz.getAnnotation(YphService.class);
